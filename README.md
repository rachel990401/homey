...
The web app gives a polished dashboard experience, while the terminal version demonstrates the same Python logic using `input()` and `print()`.
...
```bash
cd "/Users/rachel990401/Documents/New project 2"
python3 main.py
```
...
```text
homey/
├── main.py              # Entry point, Flask routes, and terminal menus
...
Both the web and terminal versions call the same Python manager classes:
...
| **While Loops** | `main.py`, `homey_utils.py` | Used for terminal menus, repeated prompts, and multi-day date checks |
| **Break / Continue** | `main.py`, `managers.py`, `storage.py` | Used in menu flow, validation, search loops, and reconciliation matching |
...
For direct Python marking, run:

```bash
python3 main.py terminal
```

For the full visual demo, run:
